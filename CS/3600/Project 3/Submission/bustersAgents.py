    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            ...
            for position in dist:
                if probability <= dist[position]:
                    maxPos = position
                    probability = dist[position]
                    distance = self.distancer.getDistance(pacmanPosition, position)
            if self.distancer.getDistance(pacmanPosition, maxPos) < closestDistance:
                closestDistance = self.distancer.getDistance(pacmanPosition, maxPos)
                closestGhostPosition = maxPos
        closestDistance = float("inf")
        for item in legal:
            distanceCheck = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, item), closestGhostPosition)
