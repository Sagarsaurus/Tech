...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for position in self.legalPositions:
            possibilities[position] = 0.0
            
        for position in self.legalPositions:
            ghostPosition = self.setGhostPosition(gameState, position)
            positionDistribution = self.getPositionDistribution(ghostPosition)
            for item in positionDistribution.items():
                possibilities[item[0]] += item[1]*self.beliefs[position]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        remainder = numLegalPositions % self.numParticles
        overallRatio = remainder/self.numParticles
        
        self.particles = []
        while(self.numParticles > len(self.particles) + len(self.legalPositions)):
            for position in self.legalPositions:
                self.particles.append(position)
        numParticles = len(self.particles)
        for i in range(self.numParticles - numParticles):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            for particle in self.particles:
                if emissionModel[util.manhattanDistance(particle, pacmanPosition)]>0:
                    possibilities[particle] += emissionModel[util.manhattanDistance(particle, pacmanPosition)]
            if possibilities.totalCount():
                toSet = []
                for positions in range(self.numParticles):
                    toSet.append(util.sample(possibilities))
                self.particles = toSet
            else:
                self.initializeUniformly(gameState)
        else:
            self.particles = []
            for position in range(self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        tempList = []
        for particle in self.particles:
            position = self.setGhostPosition(gameState, particle)
            distribution = self.getPositionDistribution(position)
            tempList.append(util.sample(distribution))
    ...
    def getBeliefDistribution(self):
        ...
        for position in self.particles:
            toAdd = 1.0/self.numParticles
            possibilities[position] += toAdd
        possibilities.normalize()
